...
def dispatch_or_admin_required(f):
    ...
    def decorated(*args, **kwargs):
        email = get_current_user()
        # 2026-08-11: dispatch and field_manager are merged (see the role sets in
        # workorders.py). get_role() resolves ONE active role, so without
        # field_manager here the same person loses the route planner and the
        # location editor whenever their active role happens to be field_manager.
        if not email or get_role(email) not in ("admin", "dispatch", "field_manager"):
            return jsonify({"error": "Unauthorized"}), 403
        return f(*args, **kwargs)
    return decorated
...
def mlh_rename_open(cursor, machine_code, new_name, lat, lon):
    """RENAME semantics: the machine did NOT move — the label for its current
    site was corrected/changed. Rewrites the OPEN interval's name in place so
    history stays continuous (no fake cutover; searches by the new name span
    the whole stay). Returns the number of rows updated (0 = no open interval).
    """
    try:
        cursor.execute("""
            UPDATE MachineLocationHistory
            SET LocationName = %s, Latitude = %s, Longitude = %s
            WHERE MachineCode = %s AND ValidToOle IS NULL
        """, (new_name, lat, lon, str(machine_code)))
        return cursor.rowcount
    except Exception as e:
        print(f"[mlh_rename_open] skipped for {machine_code}: {e}")
        return 0
...
def init_db():
    ...
    try:
        ...
        for col, dtype in new_cols:
            ...
            """)
            conn.commit()
        # Canonical de-duplicated vend feed. The machines/gateway sometimes write the
        # same physical vend more than once — occasionally under a DIFFERENT event
        # code at the exact same instant — so dedup keys on (machine, timestamp)
        # ONLY, never on the event code. One machine cannot dispense two drinks in
        # the same ~0.1s tick. ORDER BY [Event Code] makes the surviving row
        # deterministic. dbo.VendEvents is the canonical reference for ad-hoc /
        # reporting SQL; the app's hot-path queries embed the SAME dedup inline
        # (with date filters inside the window subquery) so predicates apply
        # before the window function.
        cursor.execute("""
            IF NOT EXISTS (SELECT 1 FROM sys.indexes
                           WHERE name = 'IX_MasterData_Machine_Time'
                             AND object_id = OBJECT_ID('[MasterData Table]'))
            CREATE NONCLUSTERED INDEX IX_MasterData_Machine_Time
                ON [MasterData Table] ([Machine Code], [Date Time])
                INCLUDE ([Event Code])
        """)
        conn.commit()
        cursor.execute("""
            CREATE OR ALTER VIEW dbo.VendEvents AS
            SELECT [Machine Code], [Event Code], [Date Time]
            FROM (
                SELECT [Machine Code], [Event Code], [Date Time],
                       ROW_NUMBER() OVER (
                           PARTITION BY [Machine Code], CAST([Date Time] AS FLOAT)
                           ORDER BY [Event Code] ASC
                       ) AS _rn
        ...
        try:
            ...
            if code:
                ...
                if cursor.fetchone()[0]:
                    # Machine already exists — NEVER overwrite from the static seed
                    # list. (The old UPDATE re-applied 2026-era names/coords on every
                    # deploy, silently reverting renames and completed moves.)
                    pass
                else:
                    cursor.execute(
...
def get_topups():
    ...
    try:
        conn = get_connection()
        cursor = conn.cursor()
        # Single-scan dedup join: one pass over MasterData grouped to distinct
        # (machine, instant) pairs — de-duplicates replayed vends AND is ~2x
        # faster than the old per-machine correlated COUNT(*).
        cursor.execute("""
            SELECT
                ml.MachineName,
                ml.MachineCode,
            ...
            GROUP BY ml.MachineName, ml.MachineCode, ml.LastTopupTimestamp, ml.CountBeforeLastTopup
    ...
    try:
        ...
        cursor.execute(f"""
            ...
            LEFT JOIN (
                ...
                WHERE LEN(CAST([Event Code] AS NVARCHAR(20))) = 6
                  AND CAST([Event Code] AS NVARCHAR(20)) LIKE '1%'
                  AND CAST([Machine Code] AS NVARCHAR(50)) IN ({placeholders})
                GROUP BY CAST([Machine Code] AS NVARCHAR(50)), CAST([Date Time] AS FLOAT)
            ) v ON v.code = CAST(ml.MachineCode AS NVARCHAR(50))
               AND (ml.LastTopupTimestamp IS NULL OR v.t >= ml.LastTopupTimestamp)
            ...
            GROUP BY ml.MachineCode, ml.MachineName, ml.Latitude, ml.Longitude, ml.LastTopupTimestamp
...
def update_location(code):
    ...
    mode     = (data.get("name_change_mode") or "").strip().lower()   # "rename" | "move"
    eff_str  = (data.get("effective") or "").strip()                  # move time, SGT "YYYY-MM-DD HH:MM"
    if not name:
    ...
    try:
        ...
        if not row:
            ...
            return jsonify({"error": "Machine not found."}), 404
        current_name = row[0]
        # NULL current name = initial labelling, not a site change → rename path.
        initial_label = (current_name is None)
        name_changed  = (not initial_label and name != current_name)

        # A name change MUST declare its meaning — this is what keeps location
        # history honest. RENAME = same physical site, label corrected → the
        # current stay is relabelled in place, history stays continuous.
        # MOVE = machine physically relocated → the old name's interval CLOSES
        # at the move time and the new name's interval OPENS there, so
        # searching the old location stops at the move and the new one starts.
        eff_ole = None
        if name_changed:
            if mode not in ("rename", "move"):
                conn.close()
                return jsonify({"error": (
                    "Name change requires name_change_mode: 'rename' (same site, "
                    "label corrected) or 'move' (machine physically relocated).")}), 400
            if mode == "move":
                if eff_str:
                    try:
                        eff_ole = to_ole_date(datetime.strptime(eff_str, "%Y-%m-%d %H:%M"))
                    except ValueError:
                        conn.close()
                        return jsonify({"error": "effective must be YYYY-MM-DD HH:MM (SGT)."}), 400
                else:
                    eff_ole = now_sgt_ole()
                if eff_ole > now_sgt_ole() + (5.0 / 1440.0):
                    conn.close()
                    return jsonify({"error": "Move time cannot be in the future."}), 400
                cursor.execute("""
                    SELECT TOP 1 ValidFromOle FROM MachineLocationHistory
                    WHERE MachineCode = %s AND ValidToOle IS NULL
                    ORDER BY ValidFromOle DESC
                """, (code,))
                open_iv = cursor.fetchone()
                if open_iv and open_iv[0] is not None and eff_ole <= float(open_iv[0]):
                    conn.close()
                    return jsonify({"error": (
                        "Move time predates the current stay. Use the Machine "
                        "History card's 'Record a past move' instead.")}), 400
        ...
        if new_code and new_code != code:
            ...
            # carry history forward to the new code
            cursor.execute(
        ...
        if name_changed and mode == "move":
            mlh_record_change(cursor, hist_code, name, lat, lon, "admin", at_ole=eff_ole)
        else:
            # rename, initial labelling, or unchanged name (coords touch-up):
            # relabel/refresh the open interval in place — never a cutover.
            if mlh_rename_open(cursor, hist_code, name, lat, lon) == 0:
                # no open interval exists — create one (covers new machines)
                mlh_record_change(cursor, hist_code, name, lat, lon, "admin")
        conn.commit()
        conn.close()
        return jsonify({"ok": True})
    except Exception as e:
